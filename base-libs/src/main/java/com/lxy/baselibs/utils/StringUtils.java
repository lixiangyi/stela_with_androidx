...
//        String regEx = "[ ~!@#$%^&*()_+]|\n|\r|\t";
    ...
    public static boolean verifyPassword(String password) {
        ...
        //【含有小写英文】true
        String regexlow = ".*[a-z].*";
        if (!password.matches(regexlow)) {
            return false;
        }
        //【含有大写英文】true
        String regexUp = ".*[A-Z].*";
        if (!password.matches(regexUp)) {
            return false;
        }
        //【含有数字】true
        String regex2 = ".*[0-9].*";
...
//        }
    ...
    public static boolean verifyNickName(String nickname) {
        ...
        //【含有特殊字符】false
        if (isSpecialChar(nickname)) {
            return false;
        }
        //【含有stela】false
        if (nickname.contains("stela")) {
            return false;
        }
        return true;
    ...
    public static String getString(int rid){
        return BaseApplication.getContext().getString(rid);
    }
